...
Jenkins.prototype.get_build_status =function (build_id, callback)
{
  ...
  this.jenkins.job.exists(name, function(err, exists) {
    ...
    this.jenkins.queue.list(function(err, queue) {
      if (err) return callback(err);
      if(!queue) return callback(Error('Queue is undefined'));
      ...
      this.jenkins.build.get(name, 1, function(err, data) {
        if (err) return callback(Error("Job "+ name + " does not exist"));

        var status = data["result"];

        if(status == 'SUCCESS') return callback(null, 'success');
        if(status == 'FAILURE') return callback(null, 'fail');
        if(status == null)      return callback(null, 'ongoing');

        return callback(Error("Unknow return status" + status));
...
Jenkins.prototype.build = function (build_id, yaml, baseUrl, callback)
{
  ...
  console.log(baseUrl + "/api/Build/"+build_id+"/complete");
  var config = {project :
      [{action:""},
      {description:""},
        ...
        {endpoints: [{
          ...
          ]},
          {
            "com.tikal.hudson.plugins.notification.Endpoint" : [
              {protocol: "HTTP"},
              {format:"JSON"},
              {url:baseUrl + "/api/Slaves/{slave_id}/end"},
              {event:"all"},
              {timeout:30000},
              {loglines:0}
            ]}
  ...
  this.jenkins.job.exists(name, function(err, exists) {
    if (err) return callback(err);
    if(exists) return callback(Error("Job "+ name + " already exist"));
      ...
      this.jenkins.job.create(name, xml(config), function(err) {
        ...
        //Build the job:
        this.jenkins.job.build(name, callback);
...
Jenkins.prototype.remove_node = function(id, callback)
{
  ...
  this.jenkins.node.exists(name, function(err, exists) {
    if (err) return callback(err)
    if(!exists) return callback(Error("Node "+ name + " does not exist"));
